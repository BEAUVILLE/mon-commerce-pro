...
function encaisser(){
  ...
  const {sub,rem,total}=calcTotals();
  if(total<=0){showToast("Total invalide");return}
  ...
  // Pont POS → PAY : POS garde les articles, PAY reçoit seulement la recette finale.
  const payBridgeOk = sendPosReceiptToPay(vente);

  const prods=readJSON("caisse_prods",getProds());
  cart.forEach(ci=>{
    const p=prods.find(x=>Number(x.id)===Number(ci.id));
    if(p)p.stock=Math.max(0,Number(p.stock||0)-Number(ci.qty||0));
  });
  saveProds(prods);
  ...
  document.getElementById("recuDate").textContent=
    now.toLocaleDateString("fr-FR",{weekday:"long",day:"numeric",month:"long",year:"numeric"})+
    " — "+
    now.toLocaleTimeString("fr-FR",{hour:"2-digit",minute:"2-digit"})+
    (client?" · "+esc(client):"");

  document.getElementById("recuLines").innerHTML=
    cart.map(i=>`<div class="recu-line"><span>${esc(i.emoji||"📦")} ${esc(i.name)} ×${Number(i.qty)}</span><span>${fmt(Number(i.price)*Number(i.qty))}</span></div>`).join("")+
    (rem>0?`<div class="recu-line"><span>Remise</span><span>- ${fmt(rem)}</span></div>`:"");

  setText("recuTotal",fmt(total));
  setText("recuPaymode","Paiement : "+(payLabels[selPay_]||"Paiement"));
  setText("okAmountBig",fmt(total));

  showToast(payBridgeOk ? "✅ Vente enregistrée. Recette PAY préparée." : "✅ Vente enregistrée. Ticket disponible si besoin.");
...
function nouvelleVente(){
  ...
  document.getElementById("remiseVal").value="";
  document.getElementById("searchInput").value="";
  selCat="Tous";
  selPay_="esp";
  document.querySelectorAll(".pay-mode-btn").forEach(b=>b.classList.remove("active"));
  document.getElementById("pm-esp").classList.add("active");
  buildCats();
  renderProds();
  renderAddition();
  updatePanierBar();
  updateTotals();
  showPage("page-caisse");
}
...
function clearHistorique(){
  if(confirm("Effacer tout l’historique des ventes ?")){
    saveVentes([]);
    renderStats();
...
function saveNote(){
  ...
  notes.unshift({id:Date.now(),date:new Date().toISOString(),type,amount,text});
  saveNotes(notes);
  document.getElementById("noteAmount").value="";
  document.getElementById("noteText").value="";
  renderNotes();
  showToast("Note gardée");
}
...
function renderNotes(){
  ...
  el.innerHTML=notes.length?notes.map(n=>{
    ...
    return`<div class="note-card"><div class="note-title">${noteLabel(n.type)} ${n.amount?`· ${fmt(n.amount)}`:""}</div>${n.text?`<div class="note-text">${esc(n.text)}</div>`:""}<div class="note-meta">${d.toLocaleDateString("fr-FR",{day:"numeric",month:"short",year:"numeric"})} ${d.toLocaleTimeString("fr-FR",{hour:"2-digit",minute:"2-digit"})}</div><div style="margin-top:10px"><button class="btn-danger" onclick="deleteNote(${Number(n.id)})">🗑️ Supprimer</button></div></div>`;
  }).join(""):`<div class="no-data">Aucune note. Les notes restent dans le téléphone.</div>`;
}

function noteLabel(t){
  return({vente:"Vente",depense:"Dépense",client_doit:"Client doit",stock:"Stock / achat",autre:"Autre"}[t]||"Note");
}

function deleteNote(id){
  const notes=getNotes().filter(n=>Number(n.id)!==Number(id));
  saveNotes(notes);
  renderNotes();
...
function clearNotes(){
  if(confirm("Effacer toutes les notes de ce téléphone ?")){
    saveNotes([]);
    renderNotes();
...
document.getElementById("additionOverlay").addEventListener("click",e=>{
  if(e.target===e.currentTarget)closeAddition();
});

init();

/* DIGIY — exposition explicite sur window pour l'oreille métier */
...
window.sendPosReceiptToPay = sendPosReceiptToPay;
window.openPayHandoff = openPayHandoff;
